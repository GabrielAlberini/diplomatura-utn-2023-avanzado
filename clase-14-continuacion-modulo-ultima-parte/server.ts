...
serverTCP.on("connection", (socket) => {
  ...
  socket.on("data", (data) => {
    ...
    const r = JSON.parse(stringr); // ["moduloUno", "1", "2"]
    ...
    if (r[0] === "moduloUno" && r.length === 3) {
      ...
      const array = JSON.parse(r[1]);
      const primerElemento = moduloSeis(array);
      socket.write(JSON.stringify(primerElemento));
    } else if (r[0] === "moduloSiete") {
      const numero = Number(r[1]);
      const factorial = moduloSiete(numero);
      socket.write(JSON.stringify(factorial));
    } else if (
      r[0] === "moduloOcho" &&
      r[1].startsWith("{") &&
      r[1].endsWith("}")
      ...
      const data = moduloOcho(obj);
      socket.write(JSON.stringify(data));
    } else if (r[0] === "moduloNueve" && r.length === 2) {
      const array = JSON.parse(r[1]);
      const numeroMayor = moduloNueve(array);
      socket.write(JSON.stringify(numeroMayor));
    } else {
      socket.write(JSON.stringify("Petición incorrecta"));
    }
  });
